...
class BuildCardGame {
   ...
   private void newGame() {
      ...
      this.cardFramework = new CardGameFramework(1, 0, 0, null, NUM_PLAYERS,
               CARDS_PER_HAND);
...
class BuildCardTable extends CardTable {
   ...
   public void updateTable() {
      ...
      this.leftPileBtn
               .addActionListener(new BuildCardController(gamedata, this));
...
class Hand {
   ...
   /*
    * public Card playCard(int cardIndex) { Card cardToPlay;
    * 
    * if (this.numCards > 0 && cardIndex < this.numCards) { cardToPlay =
    * myCards[cardIndex]; for (int x = cardIndex; x < numCards; x++) {
    * myCards[x] = myCards[x+1]; } numCards--; } else { cardToPlay = new
    * Card('B', Card.Suit.SPADES); } return cardToPlay; }
    */
...
class GUICard {
   ...
   private static String[] values = { "X", "2", "3", "4", "5", "6", "7", "8",
            "9", "T", "J", "Q", "K", "A" };
   /*
    * public GUICard() { if (!iconsLoaded) { loadCardIcons(); } }
    */
...
class Deck {
   ...
   public Deck(int numberOfPacks) {
      ...
      if (numberOfPacks > MAX_PACKS) {
         ...
         System.out.println(
                  "The number was greater than 6. Reducing the number to 6.");
      } else {
   ...
   public void init(int numOfPacks) {
      ...
      for (int i = 0; i < this.numPacks; i++) {
         for (int j = 0; j < masterPack.length; j++) {
            this.topCard++;
            this.cards[this.topCard] = new Card(masterPack[j].getValue(),
   ...
   // accessor for individual card and return a card with errorFlag = true if k
   // is bad
   public Card inspectCard(int k) {
      Card card;
      if (k > topCard) {
   ...
   private static void allocateMasterPack() {
      ...
      char values[] = { 'A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J',
               'Q', 'K' };
